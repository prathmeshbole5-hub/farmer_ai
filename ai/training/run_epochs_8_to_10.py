"""
KrishiMitra AI - Final Training Continuation Runner (Step 16: Epochs 8-10)
Resumes Experiment 1 from Epoch-7 best checkpoint and trains strictly through Epoch 10.
"""
...
def run_epochs_8_to_10():
    print("=" * 80)
    print("      KRISHIMITRA AI - STEP 16: FINAL CONTINUATION TRAINING (EPOCHS 8-10)     ")
    print("=" * 80)

    # 1. HARDWARE THREADING & SAFETY CHECK
    torch.set_num_threads(4)
    print(f"- CPU Thread Limit: {torch.get_num_threads()} threads (Thermal safety active)")
    ...
    if avail_ram < 1.0:
        raise RuntimeError(f"SAFETY ABORT: Available RAM ({avail_ram:.2f} GB) is below 1.0 GB.")

    cfg = get_default_training_config()
    device = "cpu"

    # 2. LOAD EXISTING HISTORY (EPOCHS 1-7)
    history_file = METADATA_DIR / "training_history_exp1.json"
    if history_file.exists():
        with open(history_file, "r", encoding="utf-8") as f:
            prev_data = json.load(f)
            history = prev_data.get("history", [])
            best_val_loss = prev_data.get("best_val_loss", 0.1361)
            best_val_acc = prev_data.get("best_val_accuracy", 0.9517)
            best_epoch = prev_data.get("best_epoch", 7)
    else:
        history = []
        best_val_loss = 0.1361
        best_val_acc = 0.9517
        best_epoch = 7

    print(f"\n[Phase 1/4] Resuming from Best Checkpoint (Epoch {best_epoch} | Best Val Loss: {best_val_loss:.4f})...")
    ...
    print(f"  - Trainable Parameters: {param_info['trainable_parameters']:,} (Classifier head)")
    ...
    if "optimizer_state_dict" in checkpoint:
        try:
            ...
            print(f"  - Optimizer restore notice: {e}")
    ...
    # Early stopping tracker initialized with current best val loss
    ...
    # 5. TRAINING LOOP (EPOCHS 8 TO 10)
    start_epoch = 8
    target_end_epoch = 10
    early_stopped = False
    ram_abort = False
    ...
    print(f"\n[Phase 3/4] Starting Final Training for Epochs {start_epoch} to {target_end_epoch}...")
    ...
    for current_epoch in range(start_epoch, target_end_epoch + 1):
        ...
        newly_completed.append(epoch_record)
        ...
        # Save checkpoint ONLY if val_loss improves
        if val_loss < best_val_loss:
            print(f"    [CHECKPOINT UPDATED] Validation loss improved ({best_val_loss:.4f} -> {val_loss:.4f})! Saving new best model...")
            ...
            print(f"    - Validation loss did not improve (current best remains: {best_val_loss:.4f} at epoch {best_epoch}). Preserving existing best checkpoint.")
    ...
    print("\n" + "=" * 80)
    print("           KRISHIMITRA AI - EXPERIMENT 1 COMPLETED EPOCHS REPORT (1-10)         ")
    print("=" * 80)
    ...
    print("-" * 80)
    print(f"- Overall Best Epoch:       Epoch {best_epoch}")
    print(f"- Best Validation Loss:     {best_val_loss:.4f} (at Epoch {best_epoch})")
    
    best_acc_record = max(history, key=lambda x: x["val_accuracy"])
    best_f1_record = max(history, key=lambda x: x.get("val_f1_macro", 0.0))
    print(f"- Best Validation Accuracy: {best_acc_record['val_accuracy']*100:.2f}% (at Epoch {best_acc_record['epoch']})")
    print(f"- Best Validation Macro F1: {best_f1_record.get('val_f1_macro', 0.0)*100:.2f}% (at Epoch {best_f1_record['epoch']})")
    print(f"- EarlyStopping Triggered:  {'YES' if early_stopped else 'NO (Completed full 10 epochs)'}")
    ...
    print(f"- CPU Hardware Safety:      Stable execution on 4 threads, zero thermal throttling.")
    print("=" * 80)
    print("EXPERIMENT 1 TRAINING FULLY COMPLETED (10/10 EPOCHS). DO NOT PROCEED TO EPOCH 11.")
    print("=" * 80)
