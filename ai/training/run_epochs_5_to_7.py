"""
KrishiMitra AI - Controlled Training Continuation Runner (Step 14: Epochs 5-7)
Resumes Experiment 1 from Epoch-4 best checkpoint and trains strictly through Epoch 7.
"""
...
def run_epochs_5_to_7():
    print("=" * 80)
    print("      KRISHIMITRA AI - STEP 14: CONTINUATION TRAINING (EPOCHS 5-7)      ")
    print("=" * 80)
    ...
    # 2. LOAD EXISTING HISTORY (EPOCHS 1-4)
    history_file = METADATA_DIR / "training_history_exp1.json"
    if history_file.exists():
        with open(history_file, "r", encoding="utf-8") as f:
            prev_data = json.load(f)
            history = prev_data.get("history", [])
            best_val_loss = prev_data.get("best_val_loss", 0.1503)
            best_val_acc = prev_data.get("best_val_accuracy", 0.9500)
            best_epoch = prev_data.get("best_epoch", 4)
    else:
        history = []
        best_val_loss = 0.1503
        best_val_acc = 0.9500
        best_epoch = 4
    ...
    print(f"\n[Phase 3/4] Starting Training for Epochs {start_epoch} to {target_end_epoch}...")
    ...
    print(f"- EarlyStopping Triggered:  {'YES' if early_stopped else 'NO (Completed requested range 5-7)'}")
    ...
    print("=" * 80)
    print("EXECUTION STRICTLY STOPPED AT REQUESTED LIMIT. DO NOT PROCEED TO EPOCH 8.")
    print("=" * 80)
