...
V3_MODEL_PATH = BASE_DIR / "models" / "soil_classifier_v3.keras"
V3_LABELS_PATH = BASE_DIR / "models" / "soil_labels_v3.json"
TEST_DIR = BASE_DIR / "Dataset" / "soil_classification" / "test"

def run_comparison():
    print("==================================================================")
    print("MODEL V2 VS MODEL V3 SIDE-BY-SIDE COMPARISON")
    print("==================================================================")
    ...
    print(f"{'Metric':<25} | {'Model V2':<12} | {'Model V3':<12} | {'Difference':<10}")
    print("-" * 68)
    ...
    for label, key in metrics_to_compare:
        ...
        v3_val = v3[key] * 100 if "accuracy" in key or "precision" in key or "recall" in key or "f1" in key else v3[key]
        diff = v3_val - v2_val
        print(f"{label:<25} | {v2_val:>10.2f}% | {v3_val:>10.2f}% | {diff:>+9.2f}%")
        
    print("=" * 68)
    print("\nPER-CLASS RECALL & F1 COMPARISON:")
    print("-" * 68)
    print(f"{'Soil Class':<20} | {'V2 Recall':<10} | {'V3 Recall':<10} | {'V2 F1':<8} | {'V3 F1':<8}")
    print("-" * 68)
    
    classes = sorted(v2["per_class_metrics"].keys())
    for cls in classes:
        ...
        print(f"{cls:<20} | {v2_rec:>8.2f}% | {v3_rec:>8.2f}% | {v2_f1:>6.2f}% | {v3_f1:>6.2f}%")
    print("==================================================================\n")
    
    # Determine the best model
    # Success Criteria: macro F1 and test accuracy
    if v3["macro_f1"] > v2["macro_f1"] and v3["test_accuracy"] >= v2["test_accuracy"]:
        print("DECISION: V3 is the new best model")
    else:
        print("DECISION: V2 remains the best model")
    print("==================================================================\n")

def run_predictions():
    print("==================================================================")
    print("MODEL V3 SAMPLE TEST INFERENCE VERIFICATION")
    print("==================================================================")
    print(f"{'Image':<45} | {'Actual Class':<15} | {'Predicted Class':<15} | {'Confidence':<10} | {'Status':<8}")
    print("-" * 105)
    
    # Load V3 model and labels
    model = tf.keras.models.load_model(str(V3_MODEL_PATH))
    with open(V3_LABELS_PATH, "r") as f:
